import { extractChangedFiles, createPRonGitHub } from "./createSmartPR.js";
import { Octokit } from "@octokit/rest";
const githubToken = process.env.GITHUB_TOKEN;
const githubOwner = process.env.GITHUB_OWNER;
const githubRepo = process.env.GITHUB_REPO;
const repoBBranch = process.env.REPO_B_BRANCH || "master";
...
export const getOpenPullRequests = async () => {
  if (!githubToken || !githubOwner || !githubRepo) {
    throw new Error(
      "Missing GitHub configuration (GITHUB_TOKEN, GITHUB_OWNER, GITHUB_REPO)"
    );
  }
    ...
    const prsWithFiles = await Promise.all(
      prs.map(async (pr) => {
        try {
          ...
          console.warn(
            `⚠️ Could not get files for PR #${pr.number}:`,
            err.message
          );
          return {
            number: pr.number,
 ...
 */
export const mergeAndCreatePRNoClaude = async (
  diffText,
  latestCommit,
  commitMessages = [],
  branchName,
